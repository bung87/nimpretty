...
proc writeOut*(em: Emitter, content: string,outFile:AbsoluteFile)  =
  ## Write to disk
 
  if fileExists(outFile) and readFile(outFile.string) == content:
    discard "do nothing, see #9499"
    return
  var f = newFileStream(outFile.string, fmWrite)
  if f == nil:
    discard
...
proc closeEmitter*(em: var Emitter,outFile: AbsoluteFile) =
  ## Renders emitter tokens and write to a file
  let content = renderTokens(em)
  em.writeOut(content,outFile)
