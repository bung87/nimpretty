...
proc openParsers*(p: var TParsers, fileIdx: FileIndex, inputstream: Stream;
...
proc setupParsers*(p: var TParsers; fileIdx: FileIndex; cache: IdentCache;
  ...
  if not open(f, filename.string):
    ...
    return false
  openParsers(p, fileIdx, newStringStream(f.readAll), cache, config)
  result = true
